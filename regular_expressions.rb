...
def grab_sin(string)
  my_match = /\d{3}-\d{3}-\d{3}$/.match(string)
  my_match.nil? ? nil : my_match.to_s
end
...
def grab_all_sins(string)
  my_match = string.scan(/\d{3}-\d{3}-\d{3}/)
  my_match.empty? ? [] : my_match
end
...
def hide_all_sins(string)
  my_match = string.scan(/\d{3}-\d{3}-\d{3}/)
  return string if my_match.empty?

  my_match = my_match.map do |x|
    x.gsub(/^\d{3}-\d{3}/, 'XXX-XXX')
  end
  my_match.join(", ")
end
...
puts "hide_all_sins has only a single SIN"
string = "please confirm your identity: 123-456-142"
puts hide_all_sins(string) == "XXX-XXX-142"

# Ensure all of the Social Insurance numbers use dashes for delimiters.
# Example: 480.01.4430 and 480014430 would both be 480-01-4430.
def format_sins(string)
  return string if (string.scan(/\d{3}\D{0,1}\d{3}\D{0,1}\d{3}/)).empty?
  my_match = string.split(", ")
